...
int main(int argc, char **argv)
{
    ...
    char *base_name = (char*)malloc(strlen(argv[1])+strlen(base_postfix)+1);
...
void analyze_CPK(reader_t *infile, const char *base_name, long file_length)
{
    ...
    {
        unsigned char buf[4];
        static const char CPK_signature[] = "CPK "; /* intentionally unterminated */
        get_bytes_seek(CpkHeader_offset, infile, buf, 4);
        CHECK_ERROR (memcmp(buf, CPK_signature, 4), "CPK signature not found");
    }

    long toc_offset = 0, itoc_offset = 0, content_offset, CpkHeader_count, start_off = 0;
    int align, band=0;
    {
    ...
    if (toc_offset)
    {
        ...
        {
            unsigned char buf[4];
            static const char TOC_signature[] = "TOC "; /* intentionally unterminated */
            get_bytes_seek(toc_offset, infile, buf, 4);
            CHECK_ERROR (memcmp(buf, TOC_signature, 4), "TOC signature not found");
        }
        ...
        for (int i = 0; i < toc_entries; i++)
        {
...
#endif
            // Check if that offset contain other .CPK file (NUNS GEN)
            unsigned char buf[4];
            static const char CPK_signature[] = "CPK "; /* intentionally unterminated */
            get_bytes_seek(file_offset+0x00, infile, buf, 4);
            
            if (!memcmp(buf, CPK_signature, 4))
            {
                printf("CPK File, extracting it... \n");
            ...
            if (extract_size > file_size && band !=1 )
            {
    ...
    else if (itoc_offset)
    {
        ...
        char file_name[256];
        sprintf(file_name, "%s", base_name);
        
        int name_len = strlen(file_name);
        name_len = (name_len - strlen(base_postfix));
        
        file_name[name_len] = '\0';
        
        strcpy(file_name+name_len, ".out");
        ...
        {
            unsigned char buf[4];
            static const char ITOC_signature[] = "ITOC"; /* intentionally unterminated */
            get_bytes_seek(itoc_offset, infile, buf, 4);
            CHECK_ERROR (memcmp(buf, ITOC_signature, 4), "ITOC signature not found");
        }
        ...
        for (int datal_i = 0, datah_i = 0; datal_i < itoc_filesl || datah_i < itoc_filesh; )
        {
            uint16_t id;
            ...
            if (datal_i < itoc_filesl && (datah_i == itoc_filesh || next_l_id < next_h_id))
            {
                /* L is 2 byte sizes */
            ...
            else if (datah_i < itoc_filesh && (datal_i == itoc_filesl || next_h_id < next_l_id))
            {
            ...
            //char *file_name = number_name("", ".bin", id, UINT16_MAX);
            //printf("%s 0x%lx %ld\n",
            //        file_name, (unsigned long)file_offset, file_size);
            
			char outfilename[256] = {0};
			sprintf(outfilename, "%s%03d", file_name, id);
            ...
            if (extract_size > file_size)
            {
                ...
                long uncompressed_size =
                    uncompress2(infile, file_offset, file_size, outfile, start_off);
                printf("   uncompressed to %ld\n", uncompressed_size);
                
                start_off = start_off + uncompressed_size;
